...
class CamObject : public TLorentzVector {
   ...
   public:
      ...
      void setPrefix(std::string in) { m_prefix=in;}
   ...
   template<typename W> W get(const char* key, ...) {
         ...
         va_list argptr;
         va_start(argptr,key);
         //static std::string myKey = key;
         //std::cout << "myKey=" << myKey << " and key=" << key << std::endl;
         W result = getInternal<W>(key, argptr);
         va_end(argptr);
         return result;
   }
   ...
   template<typename W> W get(W (CamObject::*memberMethod)()) {
      ...
      template<typename W> W getInternal(const std::string& key, va_list args) {
         ...
         boost::unordered_map<std::string,boost::any>::iterator it = m_propMap.find(key);
         if(it!=m_propMap.end()) {
...
*/
      ...
      enum PropertyLocation { UNAVAILABLE=0, MAP=1, STOREGATE=2, PARENT=3/*deprecated*/,AUXELEMENT=4,CHILD=5,TLV=6 };

      template<typename W> int contains(std::string key, bool checkMap=true, bool checkStoregate=true, bool checkAuxElement=true, bool checkChild=true, bool checkTLV=true) {
         bool isValid(false);
         if(checkTLV) TLVGetter<W>::get2(key,isValid,this); if(isValid) return TLV;
      ...
      int contains(std::string key,bool checkMap=true, bool checkStoregate=true) {
...
*/
      ...
      boost::unordered_map<std::string,boost::any>* getPropertyMap() { return &m_propMap; }

      void setPropertyMap(boost::unordered_map<std::string,boost::any>&& in) { std::swap(m_propMap,in); }
      void setPropertyMap(const boost::unordered_map<std::string,boost::any>& in) { m_propMap=in; }

      boost::unordered_map<std::string,std::string>* getDocuMap() { return &m_docuMap; }
      void setDocuMap(boost::unordered_map<std::string,std::string>&& in) { std::swap(m_docuMap,in); }
      void setDocuMap(const boost::unordered_map<std::string,std::string>& in) { m_docuMap=in; }
   ...
   protected:
      ...
      boost::unordered_map<std::string,boost::any> m_propMap;
      boost::unordered_map<std::string,std::string> m_docuMap; //filled with documentation if CamObject::Documentation=true
...
template<> int CamObject::contains<void>(std::string key, bool checkMap, bool checkStoregate, bool checkAuxElement, bool checkChild, bool checkTLV);
