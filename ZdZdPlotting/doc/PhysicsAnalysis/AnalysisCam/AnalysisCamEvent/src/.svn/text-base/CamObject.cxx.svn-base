...
std::string CamObject::applyTool(std::string toolName,std::string label,std::string option) {
...
std::string CamObject::applyService(std::string toolName,std::string label,std::string option) {
...
void CamObject::print(std::string propsToShow, bool callDynamic) {
   ...
   for(boost::unordered_map<std::string,boost::any>::iterator it = m_propMap.begin();it!=m_propMap.end();++it) {
      std::cout << "    " << it->first << " = ";
      ...
      if(it->second.type() == typeid(bool)) std::cout << boost::any_cast<bool>(it->second) << " (bool)";
         ...
         std::cout << "(" << System::typeinfoName(it->second.type()) << ")";
      }
   ...
   if(m_xAODParticle) {
      ...
      for( auto& aux : auxids) {
         ...
         std::cout << "    " << name << " = ";
         const type_info& typeinfo = *reg.getType(aux);
...
template<> int CamObject::contains<void>(std::string key, bool checkMap, bool, bool, bool, bool) {
   if(checkMap) {
...
std::string CamObject::type(const std::string& key, bool checkMap, bool checkStoregate, bool parseFunctions) {
   ...
   if(checkMap) {
      ...
      if(prop!=m_propMap.end()) {
         std::string out =System::typeinfoName(prop->second.type());
