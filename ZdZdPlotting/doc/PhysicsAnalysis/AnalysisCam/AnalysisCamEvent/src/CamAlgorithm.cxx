...
void CamAlgorithm::handle(const Incident& incident) {
   ...
   if(!m_doneFirst && incident.type() == IncidentType::BeginInputFile) {
      //this arrives one event too late!!??
      //upon next execute, we should fire the notify method. Can't fire it now because the eventstore hasn't loaded the info yet... ask Binet
      m_doNotify=true;//m_doneFirst=true;
