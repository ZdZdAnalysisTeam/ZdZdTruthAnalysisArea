...
Int_t TChannelInfo::Cutflow(const char* wildcmp, const char* groups){
   ...
   return Scan(TString::Format("$cutflow(%s)",wildcmp),groups,"%f (%ld)","$value:$entries");
}
      ...
      for(unsigned int j=0;j<groupFormulae.size();++j) {
         ...
         while(idx!=-1) {
            ...
            if(cellValue[idx+1]=='%') {
               ...
               std::stringstream a; a << results[j][k];
               cellValue = Form("%s%s%s",TString(cellValue(0,idx)).Data(),a.str().c_str(),TString(cellValue(idx+2,cellValue.Length())).Data());
